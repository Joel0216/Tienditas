...
-- Trigger para sumar stock si producto duplicado
CREATE TRIGGER tr_sumar_stock_producto_duplicado
BEFORE INSERT ON Producto
FOR EACH ROW
BEGIN
    DECLARE stock_existente INT DEFAULT 0;
    
    SELECT cantidad_disponible INTO stock_existente
    FROM Producto 
    WHERE nombre = NEW.nombre 
    AND precio = NEW.precio 
    AND id_marca = NEW.id_marca 
    AND id_dueño = NEW.id_dueño
    LIMIT 1;
    
    IF stock_existente > 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Producto duplicado detectado. Use UPDATE en lugar de INSERT.';
    END IF;
END$$
...
('Fermina puc tun', 'joelantoniopool@gmail.com', '$2b$10$hashed_password_here');
...
SELECT 'Dueños:' as tipo, COUNT(*) as total FROM Dueño
UNION ALL
